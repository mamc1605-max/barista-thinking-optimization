...
1) Custom (self-written) accuracy formula.
...
def my_accuracy(y_true: Iterable[str], y_pred: Iterable[str]) -> float:
    """Self-written accuracy formula: correct_predictions / total_predictions."""
...
def train_iteratively(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        score = my_accuracy(y_test, preds)

        print(
            f"Attempt {attempt:02d}: accuracy={score:.3f}, "
    ...
    print(
        ...
        f"with accuracy={best['score']:.3f} and alpha={best['alpha']:.2f}."
    )
